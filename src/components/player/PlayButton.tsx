...
export function PlayButton({ player, size = 'large' }) {
  return (
    <button
      type="button"
      className={clsx(
        'group relative flex flex-shrink-0 items-center justify-center rounded-full bg-emerald-700 hover:bg-emerald-900 focus:outline-none focus:ring-neutral-700 dark:focus:ring-neutral-400',
        {
    ...
    >
      ...
      {player.playing ? (
        <svg
          viewBox="0 0 22 28"
          className={clsx(
            'fill-white group-active:fill-white/80 dark:group-active:fill-black/80',
            {
              large: 'h-7 w-7',
              medium: 'h-5 w-5',
              small: 'h-4 w-4',
